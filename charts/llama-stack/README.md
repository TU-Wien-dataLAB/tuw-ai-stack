...
| `telemetry.serviceName`       | `string` | `"ogx-server"`                                                            | OpenTelemetry service name attached to OGX telemetry.                                                                                  |
| `telemetry.otlpEndpoint`      | `string` | `"http://otel-collector-opentelemetry-collector.telemetry.svc.cluster.local:4318"` | OTLP collector endpoint.                                                                                              |
| `telemetry.otlpProtocol`      | `string` | `"http/protobuf"`                                                         | OTLP transport protocol used by the exporters.                                                                                         |
...
| `affinity`                                 | `object` | `{}`                                                                                                                                 | Custom Kubernetes affinity configuration. When non-empty, this overrides `forceSameNode`. Templates in string values are evaluated with the Helm release context.                                                                                                      |
| `forceSameNode`                            | `bool`   | `true`                                                                                                                               | Requires all replicas in the release to run on the same node, allowing them to share the ReadWriteOnce PVC.                                                                                                                                                            |
